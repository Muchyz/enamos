...
pattern = re.compile(
    r'(<h3 className="text-white font-bold text-base tracking-wide mb-4">Follow Us</h3>\s*<div className="flex items-center gap-3">).*?(\n\s*</div>)',
    re.S
)
...
def repl(m):
    return m.group(1) + new_middle + m.group(2)
...
if n > 0:
    with open('src/pages/ContactPage.jsx', 'w') as f:
        f.write(new_content)
    print('Fixed — Follow Us now matches footer (Facebook, WhatsApp, Email, Call).')
else:
    print('No match — run: grep -n "Follow Us" -A 25 src/pages/ContactPage.jsx')
